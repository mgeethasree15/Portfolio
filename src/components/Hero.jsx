...
export default function Hero() {
  ...
  const { displayed: typedName } = useTypewriter(profile.name, 70, 300)
  const { displayed: typedRole } = useTypewriter(profile.roleHeadlines[role], 50, 300)
  ...
  return (
    <section id="home" className="relative grid-canvas border-b border-hairline">
      <div className="max-w-content mx-auto px-6 md:px-10 pt-16 pb-20 md:pt-24 md:pb-28">
        <div className="flex flex-col items-center text-center">
          ...
          <div role="tablist" className="inline-flex flex-wrap items-center justify-center gap-1 rounded-full p-1 mb-8" style={{ border: '1px solid #0D2540', backgroundColor: '#071428' }}>
            {roleFilters.map((r) => (
              <button key={r.id} role="tab" onClick={() => setRole(r.id)} className="px-4 py-1.5 text-sm font-medium rounded-full transition-colors" style={{ backgroundColor: role === r.id ? '#06B6D4' : 'transparent', color: role === r.id ? '#030B1A' : '#7BA8C4' }}>
                {r.label}
              </button>
            ))}
          </div>
            ...
            <div className="w-36 h-36 md:w-44 md:h-44 rounded-full overflow-hidden" style={{ border: '3px solid #06B6D4', boxShadow: '0 0 30px #06B6D455' }}>
              {profile.photo ? (
                ...
                <div className="w-full h-full flex items-center justify-center" style={{ backgroundColor: '#071428' }}>
                  <span className="font-mono text-[10px] text-center px-3" style={{ color: '#7BA8C4' }}>photo.jpg</span>
                </div>
              )}
            </div>
          ...
          {/* Typing name effect */}
          <h1 className="font-display text-[2.4rem] leading-[1.05] md:text-6xl mb-2" style={{ color: '#E0F2FF' }}>
            {typedName}
          ...
          <p className="font-mono text-xs tracking-[0.18em] uppercase mb-6" style={{ color: '#7BA8C4' }}>
            {profile.location} · Available for opportunities
          </p>

          <p className="text-base md:text-lg leading-relaxed max-w-xl mb-8" style={{ color: '#7BA8C4' }}>
            {profile.summary}
          </p>
        ...
        <div className="mt-16 -mx-6 md:-mx-10 overflow-hidden">
          <div className="flex">
            <div className="flex gap-4 animate-train whitespace-nowrap">
      ...
      <style>{`
        ...
        .animate-train { animation: train 18s linear infinite; }
